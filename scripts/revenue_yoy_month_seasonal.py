"""
Revenue YoY × 公告月份 seasonal effect

Hypothesis:
  Q 末月份公告（3/6/9/12）= 季報前夕，市場期待 → PEAD 弱（已 priced）
  Q 初月份公告（1/4/7/10）= 季報後 follow-up → PEAD 強？

或 specific month effect:
  5 月公告 4 月營收（Q1 完整）
  11 月公告 10 月營收（Q3 完整）
"""
...
def collect_events(universe, market_median):
    ...
    for i, tk in enumerate(universe):
        ...
        rev["report_month"] = rev["revenue_month"]  # 對應的月份
        ...
        for _, row in triggers.iterrows():
            ...
            if entry > 0:
                ...
                events.append({
                    ...
                    "report_month": int(row["report_month"]),  # 1-12
                    "year": sd.year,
                })
        if (i+1) % 400 == 0:
...
def analyze(events):
    ...
    print(f"\n  📊 PEAD alpha by 報導月份:")
    print(f"  {'month':<8} {'n':<6} {'alpha':<8} {'win%':<6} {'t':<7}")
    rows = []
    for m in range(1, 13):
        sub = events[events["report_month"] == m]
        ...
        print(f"  m={m:<6} {n:<6} {alpha:+.2f}%  {win:.1f}%  {t_str}  {verdict}")
        rows.append({"month": m, "n": n, "alpha": alpha, "t": t})

    # 季末 vs 季初分組
    print(f"\n  📊 季末 (3/6/9/12) vs 季初 (1/4/7/10) vs 季中 (2/5/8/11):")
    q_end = events[events["report_month"].isin([3, 6, 9, 12])]
    q_start = events[events["report_month"].isin([1, 4, 7, 10])]
    q_mid = events[events["report_month"].isin([2, 5, 8, 11])]
    for label, sub in [("季末 (3/6/9/12)", q_end),
                        ("季初 (1/4/7/10)", q_start),
                        ("季中 (2/5/8/11)", q_mid)]:
        n = len(sub)
        alpha = sub["fwd_60d"].mean() - sub["baseline_mean"].mean()
        ...
        t = alpha / (bs/np.sqrt(n)) if bs > 0 else None
        t_str = f"{t:+.2f}" if t else "n/a"
        print(f"  {label:<25} n={n}, alpha={alpha:+.2f}%, t={t_str}")


def main():
    print("=" * 80)
    print("  Revenue YoY × 月份 seasonal effect")
    print("=" * 80)
